...
//				return $ThumbPrefix.$RandomNumber.'.'.$ImageExt;
				...
				//return json_encode('{ "image_name":	"'.$ThumbPrefix.$RandomNumber.'.'.$ImageExt.'" }');
				return json_encode($data_array);

				//return json_encode(array('image_name' => $ThumbPrefix.$RandomNumber.'.'.$ImageExt, "image_width"	=> $this -> NewWidth, "image_height"	=> $this -> NewHeight));


/*
				echo '<table width="100%" border="0" cellpadding="4" cellspacing="0">
				    <tr><td align="center"><img src="uploads/'.$ThumbPrefix.$RandomNumber.'.'.$ImageExt.'" alt="Thumbnail"><br />'.$ThumbPrefix.$RandomNumber.'.'.$ImageExt.'</td></tr><tr>
				    <td align="center"><img src="uploads/'.$RandomNumber.'.'.$ImageExt.'" alt="Resized Image"><br />'.$RandomNumber.'.'.$ImageExt.'</td></tr></table>';

				if(isset($_POST))
				{
				    echo '<div style="text-align:left;">Other Post Variables: <pre>';
				    print_r($_POST);
				    echo '</pre></div>';
				}
*/
	 ...
	 * @return string $tempalate	This is source add new record tempalate
	 */
	function resizeImage($CurWidth,$CurHeight,$MaxWidth,$MaxHeight,$DestFolder,$SrcImage,$quality) {
